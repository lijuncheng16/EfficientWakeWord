#-*- coding: utf-8 -*-
"""
Created on Mon Nov 25 12:01:14 2019

REPLACE transform.wav in transforms folder with this one


@author: hashi

Adapted by Cody Berger and Karthik Ganesan (CMU) 2022 
"""
import os
import argparse
import pyaudio
...
filename = "checkpoints/sheila/effnetv2_xl/best-f1-model-sheila.pth"
...
'''
Note that inference timing is calculated using time.time() for all modules.
If using CUDA, run torch.cuda.synchronize() to ensure CPU activity waits for
all CUDA kernels to complete. Currently, no similar tool exists for MPS.
'''
...
# Each model input data duration in seconds, need tobe an integer numbers of chunk_duration
feed_duration = 1
feed_samples = int(fs * feed_duration)

assert feed_duration/chunk_duration == int(feed_duration/chunk_duration)

#Starting point in audio data queue for most recent chunk
...
def detect_triggerword_spectrum(inpt, timeTotal):
    ...
    timeTotal -- total time spent running the model during demo    predictions -- flattened numpy array to shape (number of output time steps)
    """
...
try:
    ...
    while run:
        ...
        voice_feature_transform = Compose([ToMelSpectrogramRT(n_mels=32, sample_rate=fs),
                                           ToTensorRT()])
